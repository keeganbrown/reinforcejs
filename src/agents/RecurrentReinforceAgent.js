...
RecurrentReinforceAgent.prototype = {
  reset: function() {
    ...
    this.actorLSTM = Recurrent.initLSTM(this.ns, [this.nh], this.na);
    this.actorG = new Recurrent.Graph();
    this.actorPrev = null;
    this.actorOutputs = [];
    this.rewardHistory = [];
    this.actorActions = [];

    this.baselineLSTM = Recurrent.initLSTM(this.ns, [this.nhb], 1);
    this.baselineG = new Recurrent.Graph();
  ...
  act: function(slist) {
    ...
    var actorNext = Recurrent.forwardLSTM(this.actorG, this.actorLSTM, [this.nh], s, this.actorPrev);
    this.actorPrev = actorNext;
    var amat = actorNext.o;
    this.actorOutputs.push(amat);

    // forward the baseline LSTM
    var baselineNext = Recurrent.forwardLSTM(this.baselineG, this.baselineLSTM, [this.nhb], s, this.baselinePrev);
  ...
  learn: function(r1) {
    ...
    if(n >= nup) {
      ...
      Recurrent.updateNet(this.actorLSTM, this.alpha); // update actor network
      Recurrent.updateNet(this.baselineLSTM, this.beta); // update baseline network
