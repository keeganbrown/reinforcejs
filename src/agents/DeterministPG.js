...
DeterministPG.prototype = {
  reset: function() {
    ...
    this.actorNet.W2 = new Recurrent.RandMat(this.na, this.ns, 0, 0.1);
    this.actorNet.b2 = new Recurrent.Mat(this.na, 1, 0, 0.01);
    this.ntheta = this.na*this.ns+this.na; // number of params in actor

    // critic
    this.criticw = new Recurrent.RandMat(1, this.ntheta, 0, 0.01); // row vector
  ...
  utilJacobianAt: function(s) {
    var ujacobian = new Recurrent.Mat(this.ntheta, this.na);
    for(var a=0;a<this.na;a++) {
      Recurrent.netZeroGrads(this.actorNet);
      var ag = this.forwardActor(this.s0, true);
      ag.a.dw[a] = 1.0;
      ag.G.backward();
      var gflat = Recurrent.netFlattenGrads(this.actorNet);
  ...
  learn: function(r1) {
    ...
    if(!(this.r0 == null)) {
      var Gtmp = new Recurrent.Graph(false);
      // dpg update:
